...
| `features/forecasts/forecast.component.ts` (+`forecast-api.service.ts`) | Экран «Прогнозы» (тестовый, на месте заглушки `forecasts`): таблица поездов с прогнозными полями Stage 3/4 (состав · станция назначения · расчёт МСК · прогноз МСК · прогноз ЖД · mistake), фильтр по терминалу, ЗЕЛЁНАЯ подсветка плановых поездов (нитка из плана). Данные — `GET /dislocation/forecast`. |
