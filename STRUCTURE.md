...
| Файл | За что отвечает |
|---|---|
| `features/dislocation/dislocation.component.ts` | Экран «Дислокация»: статус-панель системы сверху (переиспользует `plan-status-panel`) + компактный приём ЛК (тулбар «Загрузить/Обновить», список файлов → «Обработать в снимок») + кнопка «Обновить из АСУ» (ручной забор); результат — свёрнутая сводка с «подробнее». |
