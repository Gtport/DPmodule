...
| `lk_process.go` | **Приём ЛК, шаг 2**: парсинг принятых файлов и атомарная замена снимка дислокации. |
...
| `plan_sf_process.go` | Двухфазная загрузка плана с выбором групп с.ф. (`Prepare`/`Confirm`/`Touch`), pending-токены, ре-валидация против текущего снимка. |
...
| `features/plan/plan.component.ts` | Экран «План подвода»: загрузка xlsx-плана, таблица с портами/«Составом»/«Остаток на 18:00», выбор загрузки из истории, диалог выбора групп с.ф., статус-панель актуальности сверху. |
| `features/plan/plan-api.service.ts` | Обмен с бэкендом по плану: типы и запросы загрузок, ниток, ячеек портов, с.ф.-кандидатов, статуса актуальности. |
