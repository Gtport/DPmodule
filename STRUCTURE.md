...
| `journal.go` | `Journal` — запись единого журнала событий (`disl_update`/`plan_upload`): «кто» из JWT в контексте, «когда» из `clock.Now()`, `doc_ts` — время из документа; best-effort, nil-safe. |
...
| `plan_upload.go` | План подвода: загрузка файла, получение свежего/по id, история загрузок. |
...
| `features/plan/plan.component.ts` | Экран «План подвода»: загрузка xlsx-плана, таблица с портами/«Составом»/«Остаток на 18:00», выбор загрузки из истории. |
| `features/plan/plan-api.service.ts` | Обмен с бэкендом по плану: типы и запросы загрузок, ниток, ячеек портов. |
