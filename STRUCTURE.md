...
>
> Срез: **2026-07-10** (после PR #46 — таблица плана подвода, PR #45 — multi-upload ЛК).
...
| `plan_sf_process.go` | Загрузка плана с человеко-в-цикле: `Prepare` (кандидаты с.ф. + проблемные нитки без вагонов), `Revalidate` (сухой пересчёт с ручными правками индексов, снимок не трогаем), `Confirm` (правки + выбор групп с.ф.), `Touch`; pending-токены (`peek`/`take`), ре-валидация против текущего снимка. Ручная правка = `IndexPp/IsSf` на копии ниток, движок `planmatch` не тронут. |
...
| `features/plan/plan.component.ts` | Экран «План подвода»: загрузка xlsx-плана, таблица с портами/«Составом»/«Остаток на 18:00», выбор загрузки из истории, диалог проверки (группы с.ф. + ручные правки индексов ниток с итеративным `revalidate`), статус-панель актуальности сверху. |
| `features/plan/plan-api.service.ts` | Обмен с бэкендом по плану: типы и запросы загрузок, ниток, ячеек портов, с.ф.-кандидатов, проблемных ниток, `prepare`/`revalidate`/`confirm`, статуса актуальности. |
