...
| Файл | За что отвечает |
|---|---|
| `features/dislocation/dislocation.component.ts` | Экран «Дислокация»: двухшаговый приём ЛК (загрузка xlsx, в т.ч. несколько файлов + drag&drop → контроль → «Обработать в снимок»). |
| `features/dislocation/dislocation-api.service.ts` | Обмен с бэкендом по ЛК: типы и запросы приёма/контроля/обработки. |
...
| `features/plan/plan-status-panel.component.ts` | Статус-панель (образец gtport StatusPanel): способ/время обновления дислокации по терминалам + актуальность планов МА/НК; цвет чипа по возрасту; автообновление раз в минуту. |
