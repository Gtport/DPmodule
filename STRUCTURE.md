...
| Файл | За что отвечает |
|---|---|
| `stage4.go` | Чистый алгоритм раскладки поездов по ниткам станции: план-якорь, беспланные по свободным слотам с интервалом `вагонов×24/pc_рода`, пул слотов на станцию, настраиваемый допуск. Два метода раскладки (профиль станции `distribution_method`): `excel` — общий пул + глобальная сортировка по Rasch (АЭ/ГУТ-2); `staircase` — последовательная лестница с ре-якорем `currentTime` на назначенную нитку, допуск −6ч только к Rasch, стартовая нитка — жёсткий низ (УТ-1, воспроизводит эталонный `distributeGroupWithInterval`). Без БД/часов (входы параметрами). |
