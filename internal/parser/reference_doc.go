...
// ReferenceDocBatch — результат разбора ответа
// <client>/reference?number=<n>&date_create=<d>.
//
// ⚠️ Ответ на запрос по номеру — НЕ один документ. Провайдер хранит памятки
// пачками, как они к нему приехали, и по тройке находит пачку-контейнер, отдавая
// её целиком (боевая проверка 28.07.2026: attis number=11013 и number=11017 дают
// байт в байт один ответ из 5 документов; nmtp — 18). Тело ответа контракт от
// 30.07.2026 не менял; несуществующая тройка даёт HTTP 404 с телом
// {"error":"pamyatka not found"} — до разбора не доходит, его отбивает
// HTTP-клиент. Нужный документ выбирается из пачки методом [ByNumber]: номера
// внутри пачки различны (сверено 30.07.2026 на attis/11387 — 5 документов,
// номера 11387/11389/11391/11392/11393), а повторяется номер только МЕЖДУ
// датами, и они теперь разведены самим запросом.
