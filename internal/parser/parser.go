// Package parser переносит парсеры входных данных GTport (JSON из АСУ, Excel из
// ЛК, ответ запроса 601) в чистые преобразования «сырой вход → доменные модели».
// Парсеры не зависят от БД и репозиториев: на вход — байты/файл, на выход —
// []domain.Dislocation / []domain.VagonOperation. Обогащение (Stage 1–4) и
// персистентность — отдельные слои.
package parser

import (
	"fmt"
	"strings"
	"time"
)

// SourceProfile — настройки конкретного источника данных, которые в GTport были
// «зашиты» в код, а в универсальном DPmodule приходят из настроечной таблицы
// data_source (гибрид: обычные колонки + JSONB). Пока профиль передаётся
// параметром в парсер; позже его будет отдавать репозиторий источников.
//
// В профиль парсера кладём только то, что нужно самому разбору файла. Правила
// идентификации файла (маркеры «Личный кабинет»/«Техническое состояние») и
// мэппинги ОКПО→имя относятся к слою приёма/идентификации, а не к парсеру, и
// живут отдельно — здесь их нет намеренно.
type SourceProfile struct {
	// DateCutoffHour — порог часа для правила «час ≥ N → дата рейса +1 сутки»
	// (бизнес-смещение операционных суток предприятия). В GTport зашито 18.
	DateCutoffHour int
	// HeaderMarker — текст, по которому в листе Excel ищется строка заголовка
	// таблицы. В GTport зашито «Номер вагона».
	HeaderMarker string
}

// DefaultProfile возвращает профиль с поведением GTport «как есть» — для
// golden-тестов и предприятий со стандартным потоком выгрузки из ЛК РЖД.
func DefaultProfile() SourceProfile {
	return SourceProfile{
		DateCutoffHour: 18,
		HeaderMarker:   "Номер вагона",
	}
}

// generateDeterministicID — детерминированный ID записи дислокации по ключевым
// полям: "vagon/codeStationNach/DD.MM.YYYY". Стабилен между загрузками одного
// рейса. При отсутствии ключа — временный ID (temp_<unixnano>).
func generateDeterministicID(vagon, codeStationNach string, dateNach *time.Time) string {
	if vagon == "" || codeStationNach == "" || dateNach == nil || dateNach.IsZero() {
		return fmt.Sprintf("temp_%d", time.Now().UnixNano())
	}
	return fmt.Sprintf("%s/%s/%s", vagon, codeStationNach, dateNach.Format("02.01.2006"))
}

// safeSubstring безопасно извлекает подстроку [start; start+length) без паник.
func safeSubstring(s string, start, length int) string {
	if start >= len(s) {
		return ""
	}
	end := start + length
	if end > len(s) {
		end = len(s)
	}
	return s[start:end]
}

// normalizeCyrillic заменяет латинские омоглифы на кириллические (частая беда
// выгрузок АСУ: «C» латинская вместо «С» кириллической и т.п.).
func normalizeCyrillic(text string) string {
	if text == "" {
		return text
	}
	replacements := map[rune]rune{
		'A': 'А', 'B': 'В', 'C': 'С', 'E': 'Е', 'H': 'Н', 'K': 'К', 'M': 'М',
		'O': 'О', 'P': 'Р', 'T': 'Т', 'X': 'Х', 'Y': 'У',
		'a': 'а', 'c': 'с', 'e': 'е', 'o': 'о', 'p': 'р', 'x': 'х', 'y': 'у',
	}
	return strings.Map(func(r rune) rune {
		if replacement, exists := replacements[r]; exists {
			return replacement
		}
		return r
	}, text)
}
