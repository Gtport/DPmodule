...
type Config struct {
	...
	// MapView — экран «Карта» целиком (решение владельца 07.08.2026): выключен →
	// к БД тайлов не подключаемся, роуты карты не регистрируются, фронт прячет
...
// EnabledOrDefault — карта включена, пока её явно не выключили.
func (m MapView) EnabledOrDefault() bool { return m.Enabled == nil || *m.Enabled }
