...
// Location — московская зона для тех немногих мест, которым нужна НЕ доменная
// шкала, а настоящее время с зоной. Такое место в проекте одно: штамп строки
// лога (zap ставит его сам через time.Now(), то есть в зоне сервера, а VPS во
// Франкфурте). Отдаём зону наружу, чтобы логгер не заводил свой FixedZone —
// иначе мест с часовым поясом станет два, и инвариант «единственное место»
// перестанет быть проверяемым.
//
// Для бизнес-времени это НЕ вход: там по-прежнему Now() и LocalTime без зоны.
func Location() *time.Location { return moscow }
