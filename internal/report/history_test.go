...
func TestHistoryXLSX(t *testing.T) {
	...
	// Две колонки выгрузки, «ПП» убрана (решения владельца 14.08.2026):
	// факт МСК со временем + ЖД-сутки.
	assert.Equal(t, "Выгружен (факт)", got[0][17])
	assert.Equal(t, "Выгружен (ЖД)", got[0][18])
	assert.Equal(t, "09.07.2026 00:00", got[1][17], "факт выгрузки — МСК со временем")
	assert.Equal(t, "10.07.2026", got[1][18], "учётная выгрузка — ЖД-сутки")
