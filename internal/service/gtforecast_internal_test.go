...
// Свободные нитки — только в горизонте прогноза: план подвода хранится целиком
// и покрывает прошедшие сутки, их слоты в выдачу не попадают; слоты
// материализуются по ЖД-СУТКАМ плана, а не по календарным дням ниток
// (замечание владельца 07.08.2026 — свежий план на ЖД 07.08 давал «нитки за
// вчера» 06.08 утром и лишний вечер 07.08 = ЖД 08.08); занятые ниткой плана
// слоты исключаются.
func TestFreeSlotsInHorizon(t *testing.T) {
	...
	slots := []domain.NitkaSlot{{Hour: 9, Minute: 0}, {Hour: 21, Minute: 0}}
	// Свежий план на ЖД-сутки 07.08 (боевой сценарий 07.08.2026): дата PlanJd —
	// ЖД-сутки листа, физически нитки лежат на двух календарных днях (вечер
	// 06.08 и утро 07.08). Нитка 09:00 занимает утренний слот своих ЖД-суток.
	nitki := []domain.PlanNitka{
		{PlanJd: lt("2026-08-07T20:30:00")}, // физически вечер 06.08
		{PlanJd: lt("2026-08-07T09:00:00")}, // занимает слот 09:00
		{IsOstatok: true, PlanJd: lt("2026-08-07T21:00:00")}, // служебная — слот НЕ занимает
	}
	start, _ := time.Parse("2006-01-02", "2026-08-07") // горизонт: расчётные 07.08–08.08
	got := freeSlotsInHorizon(slots, nitki, start, 2)  // физически 06.08 18:00 … 08.08 18:00

	// За бортом: всё календарное утро 06.08 (ЖД 06.08 — план его не покрывает),
	// вечер 07.08 (ЖД 08.08 — плана на эти сутки нет), слот 09:00 ЖД 07.08
	// (занят ниткой). Остаётся один свободный слот ЖД-суток плана.
	want := []string{
		"2026-08-06T21:00:00", // слот 21:00 ЖД-суток 07.08 (физически вечер 06.08)
	}
	...
	if len(got) != len(want) {
		...
		t.Fatalf("слотов %d (%v), ожидалось %d (%v)", len(got), have, len(want), want)
	}
	for i, w := range want {
		if g := time.Time(got[i].Msk).Format("2006-01-02T15:04:05"); g != w {
			t.Errorf("слот %d: %s, ожидался %s", i, g, w)
		}
	}
	// ЖД-подпись вечернего слота — следующие ЖД-сутки (час ≥ 18 → +1).
	...
	stale := []domain.PlanNitka{
		{PlanJd: lt("2026-08-06T20:00:00")},
		{PlanJd: lt("2026-08-06T05:00:00")},
	}
	if got := freeSlotsInHorizon(slots, stale, start, 2); len(got) != 0 {
		t.Errorf("залежавшийся план: слотов %d, ожидалось 0", len(got))
	}
}
