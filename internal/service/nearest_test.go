...
func TestNearestTrains(t *testing.T) {
	...
	// Поезд A — плановый (позже), поезд B — только прогноз (раньше),
	// C — прибывший (исключён), D — кандидат-9 (исключён), E — чужой терминал.
	...
	repo := &fakeDislRepo{current: []domain.Dislocation{a1, a2, b1, c1, d1, e1}}
	...
	require.Len(t, trains, 2) // C (10), D (9) исключены; E — чужой терминал
	// B раньше (09:00 прогноз) — первым; A план 18:00 — вторым, с меткой плана.
