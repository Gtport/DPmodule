...
// restoreLineage — восстановление данных рейса вернувшемуся из пропажи вагону
// из его записи-8: родословная (index_main — индекс формирования, предыдущий
// индекс, накладная формирования), план-поля и ОПЕРАТОРСКИЕ решения (перестановка
// naznach, переадресация) — поток РЖД их не знает, а carry-over вагона не видел.
// Только для ТОГО ЖЕ рейса (совпадает ID = вагон+станция+дата погрузки) — новый
// рейс после новой погрузки чужие данные не наследует.
func restoreLineage(r, old *domain.Dislocation) {
	...
	// Операторские решения («снимок вернее потока»): перестановка назначения и
	// переадресация переживают пропажу, как переживали бы обычный carry-over.
	if old.Naznach != "" && old.Naznach != r.Naznach {
		r.Naznach = old.Naznach
	}
	if old.PereadrType != "" {
		r.PereadrType = old.PereadrType
		r.PereadrPort = old.PereadrPort
	}
}
