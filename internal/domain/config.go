...
type StatusPolicy struct {
	...
	// Автоочистка завершённых (поднятых) бросков старше N суток; 0 → выключена.
	// ⚠️ Ограничивает глубину отчёта по броскам: данные старше срока удаляются.
	BrosCleanupDays int `json:"bros_cleanup_days"`
	// Автоочистка закрытых эпизодов задержек (vagon_delay) старше N суток
	...
	// «Долгостой» карточки «Работа»: сколько часов с прибытия вагон должен
	// простоять на станции назначения, чтобы попасть в список; 0 → 48.
	// Отсчёт строго от date_prib (решение владельца 11.08.2026): простой РЖД
	// (prost_dn/prost_ch) для этого не годится — подача его обнуляет, и вагон,
	// который переставляют раз в сутки, не попал бы в список никогда.
	DestStandHours int `json:"dest_stand_hours"`
...
// DestStandHoursOrDefault — порог долгостоя на станции назначения с дефолтом
// 48 часов (решение владельца 11.08.2026). По истории за 30 суток нормальный
// цикл «прибытие → выгрузка» укладывается в 12–18 ч медианы, 48 ч — это 95-й
// процентиль, то есть выброс, а не медленный день.
func (p StatusPolicy) DestStandHoursOrDefault() int {
	if p.DestStandHours > 0 {
		return p.DestStandHours
	}
	return 48
}
...
type PlanProfile struct {
	...
	MaxTrainLength       int      // лимит длины состава станции (ваг) для формулы интервала Stage 4; 0 — без лимита
}
