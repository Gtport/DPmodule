...
// PorozhCargo — справочник кодов груза порожняковых рейсов (миграция 000057).
// Семейство ЕТСНГ 421 «Вагоны, перевозимые на своих осях» + 693157. Участвует
// в критерии «порожний под погрузку» (§ жизненный цикл статусов): изначально
// порожний вагон в адрес порта живёт обычным деревом статусов движения, а не 6.
type PorozhCargo struct {
	Kod     int64
	Name    string // наименование по ЕТСНГ (информационное)
	Enabled bool   // выключенный код в критерии не участвует
}
...
type Marka struct {
	...
	Color      string // цвет отображения вагонов этой атрибуции в UI (hex; '' — нет)
	Sprav1     string // запасное поле «на будущее» (пока не используется)
}
...
type Ports struct {
	...
	// NmtpNorm — норма вагонов на сети для блока «Нагрузка на ж/д сеть»
	// НМТП-отчёта (из файлов порта: ГУТ-2 2600, УТ-1 3500); 0 — не считать.
	NmtpNorm int
	// OrgShort — короткое имя организации для статус-панелей (АТТИС/НМТП);
	// пусто — панель подписывает ветку терминалами (000060).
	OrgShort string
}
