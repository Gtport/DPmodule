...
`_reference/seed/` вне git — каталог создаётся руками в клоне 2:

- копией из клона 1: `cargo.csv`, `cargo_operations.csv`, `route_speed.csv`,
  `stations.csv` (дополнить станциями региона нового клиента), `sf.csv`;
- своими: `ports.csv` — **одна строка** (ОКПО, станция, `plan_code` ПУСТОЙ,
  `pc_*` от владельца, цвет), `port_cargo_line.csv` — линии учёта терминала,
  `marka.csv` и `naznach_station.csv` — из выгрузки АНБ;
- пустыми (только строка заголовка): `max_chat.csv`, `max_route.csv`.

```bash
cd /home/alex/projects/DPmodule2
psql "$PG_DSN" -v ON_ERROR_STOP=1 -f scripts/seed_directories.sql
cp scripts/clients/_template.sql scripts/clients/river.sql   # заполнить и закоммитить
...
`plan_profile`, `nitka_schedule`, `nmtp_*` новому клиенту НЕ заполнять: пусто =
функции скрыты, Stage 4 считает прогноз по перерабатывающей способности.
В `client_settings.ingest_policy` ослабить `max_staleness_minutes` (например 720):
дефолтные 60 минут отвергнут выгрузку, снятую час назад, а грузить будут руками.
