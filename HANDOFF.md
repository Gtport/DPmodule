...
- **VPS:** Frankfurt, `147.45.216.32`, юзер `alex` (sudo). Проекты в `/home/alex/projects`.
  (Переезд на новую машину 2026-07-10; старый адрес был `147.45.216.229`.)
- **Службы:** nginx (единственный вход из интернета, домен `95850.koara.live`→фронт;
  пока A-запись у koara не исправлена — смотрим по `http://147.45.216.32`, HTTPS ждёт),
...
- **nginx (`/etc/nginx/sites-available/95850.koara.live`, исходник-копия
  `/home/alex/projects/95850.koara.live.conf`):** `location /api/` содержит
...
- **Dev-Keycloak тестовые юзеры** (`deploy/keycloak/realm-iqport.json`, контейнер
  ...
  (относительный `/realms/…`), поэтому same-origin работает на любом хосте (IP/домен)
  без пересборки; заход напрямую на `127.0.0.1:4200` мимо nginx по-прежнему без Keycloak.
- **Команды:**
  ```bash
  go build ./...                 # сборка
  go vet ./...                   # статанализ
...
- Новую работу — от свежего `main`: `git checkout main && git pull --ff-only`. Коммитим
  прямо в `main` (соло, с 2026-07-27); ветка — только под многозаходную или рискованную
  задачу. Порядок — скил `/commit`.
