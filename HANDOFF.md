...
> Документ-передача для **нового ассистента**, продолжающего работу после переезда.
> Самодостаточен: читается вместе с `CLAUDE.md`, `TARGET.md`, `PROJECT_INSTRUCTIONS.md`
> и **`STRUCTURE.md`** (карта проекта файл-за-файлом: где что лежит в backend/frontend —
> начни с неё для навигации по коду; её правило — держать в актуальном состоянии).
...
### Унификация плана подвода + решения владельца (обсуждение 2026-07-10)
Как унифицировано: **станция = данные, не код**. Один парсер `internal/parser/plan/grid.go`
(формат «новой формы» общий); отличия станции — в профиле `profile.go`
(`Profile{PlanCode, OurTerminals, MatchRequiresNaznach}`, встроенные `builtinProfiles`
`ma`/`nk`); целевые площадки матча — из `ports.plan_code` (`DirectoryCache.TargetNaznach`);
`registry.go` — для принципиально иного формата листа (свой `Parser`, иначе generic).

**Новая станция сегодня** (тот же формат) = 3 правки, БЕЗ ядра парсера: (1) профиль в
`builtinProfiles`; (2) станция в справочник `ports` (`plan_code`); (3) строка в статичный
`STATION_OPTIONS` (`frontend/.../plan/plan.component.ts`) — эндпоинта «список кодов плана» нет.

**Решения владельца:**
- **Аналитика НЕ считается** (и не нужна): «полезное/полное», «простой порта», «Перераб.
  спос.», «Заказ» не переносим. Из плана берём только строку «Остаток на 18:00» (числа
  портов как есть).
- Из «аналитики» нужны **только неиспользуемые нитки** = нитки без вагонов (`matched=false`).
  Данные уже есть (`matched`/`activ`) — нужен лишь фильтр/подсветка на фронте (правка только
  отображения, бэкенд трогать не надо).
- **Следующий шаг (решено):** вынести портрет станции в **настроечную таблицу** — профиль
  (`OurTerminals`, `MatchRequiresNaznach`, код+название, связь с `ports`) из `profile.go` в
  БД, переключить `ResolveProfile` на БД (задел уже заложен в комментарии), отдать список
  станций фронту эндпоинтом и убрать статичный `STATION_OPTIONS`. Тогда новая станция = данные,
  без пересборки. NB: «расписание ниток» (содержимое плана в `plan_nitka`) — это оперативные
  данные из файла, в настроечную таблицу НЕ выносится; в таблицу идёт только профиль станции.
  Ложится в канон настроечных таблиц (см. память `settings-tables-architecture`). Не начато.
...
Если память потеряется — этого HANDOFF + `STRUCTURE.md` + кода + `gtlogic`-эталона
достаточно, чтобы восстановить контекст и продолжить.

⚠️ **При смене ассистента/машины:** файлы в `~/.claude/.../memory/` — локальные, НЕ в git,
и другой ассистент их формат может не читать. Поэтому источник правды при передаче —
**репозиторные доки** (`HANDOFF.md`, `STRUCTURE.md`, `CLAUDE.md`, `PROJECT_INSTRUCTIONS.md`,
`TARGET.md`), они переезжают с клоном. Память — приятное дополнение, не обязательное.
...
- `main` — влиты: движок+план P-1a/P-2a/P-2b/P-1b (#39/#41/#42), обвязка/фронт-шаблон
  ...
  плана: порты/Состав/Остаток/история (#46), обновления HANDOFF (#44/#47),
  **`STRUCTURE.md` + правило поддержки в `CLAUDE.md` (#48)**. Всё из §4/§5/§5a/§5b — в `main`.
- **Открытых PR нет. Рабочее дерево чистое, всё в `main`** — точка передачи чистая.
- **NB:** миграция `000015` уже применена на боевом `dpport` (при проверке #46). В другом
  клоне/новой БД накатить: `go run ./cmd/migrate -dir migrations up`.
