...
> Дата среза: **2026-07-09**. Последняя работа — подсистема «план подвода»
> (P-1a парсер, P-2a матч, P-2b запись в снимок, P-1b хранение сетки для фронта).
> P-2b **проверен на живом dpport** (491 вагон застолблён); P-1b — сетка в БД, проверена.
...
- Проверено на dpport: сетка совпадает с разбором, повторный прогон идемпотентен.
  **HTTP GET не смоук-тестился** (порт 8080 был занят) — при желании проверить curl'ом.
...
1. **Смоук-тест HTTP GET плана** (не проверялся — порт 8080 был занят):
   `curl http://127.0.0.1:8080/api/v1/dislocation/plan/ma` (если Keycloak выключен —
   без токена). Прогон записи, если нужно перезалить: `set -a; . ./.env; set +a;
   go run ./cmd/planapply "/home/alex/projects/new_go/Мыс Астафьева.xlsx" ma` (пишет в
   снимок + сетку `plan_nitka`, обратимо). Дождаться мёржа PR #42 (P-1b) владельцем.
...
- `main` — влиты P-1a+P-2a (PR #39), P-2b (#41), обвязка/фронт-шаблон владельца (#40).
- **`feat/plan-nitka-table`** — P-1b (хранение сетки), **PR #42 открыт**, ждёт мёржа владельцем.
- Новую работу начинать от свежего `main` (после мёржа #42): `git checkout main &&
