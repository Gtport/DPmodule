...
## Шаг 6 (отдельным решением): `vagon_history`
Снимок и история — разные хранилища. В истории `naznach` пишется при INSERT и потом
только на СМЕНЕ СТАТУСА (`historyUpdateFields`: `if ps == ns { return fields }`, ветка
`ns == 10`). Значит у вагонов, которые уже прибыли/выгружены и стоят на 10/12, история
после правки снимка остаётся со старым терминалом и кормит им отчёты
«Прибывшие»/«Погрузка»/«Факт. перестановки». Правка — отдельное решение владельца:
это бизнес-история, а не снимок.

⚠️ **Матчить строго по `trip_key`, не по `id`** (`trip_key` = `vagon::bigint*100000 +
(date_nach_d::date - DATE '1970-01-01')`, у истории это generated-колонка):

```sql
UPDATE dpport.vagon_history h SET naznach = '<терминал>'
FROM dpport.naznach_fix_backup_ГГГГММДД b
JOIN dpport.dislocation d ON d.vagon = b.vagon
WHERE b.naznach_new = '<терминал>'
  AND h.trip_key = d.vagon::bigint*100000 + (d.date_nach::date - DATE '1970-01-01')
  AND h.naznach <> '<терминал>';
```
Перед этим — показать затрагиваемые строки и проверить, что у вагона в истории нет
ДРУГИХ рейсов, которые могли бы попасть под правку. Бэкап — отдельной таблицей, как для
снимка. После правки конвейер её не откатывает (обновление только на смене статуса).

⚠️ `place_vigr` — **копия `naznach`**, а не самостоятельный факт ([history.go:254](../../../internal/service/history.go#L254)
при INSERT, [history.go:201](../../../internal/service/history.go#L201) на переходе в 12).
Поправив в истории только `naznach`, вы оставите у выгруженных вагонов
`place_vigr` со старым терминалом — и отчёт «Факт. перестановки» (соответствие =
«выгружен на терминале назначения») запишет их в НЕсоответствие. Решать вместе с
`naznach`: либо править оба поля, либо осознанно оставить расхождение как факт.
