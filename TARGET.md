...
- **Обнуление:** `gruzpol_s = "0"`, `naznach = "0"` — и в снимке (вагон к нам не
  доедет, в выборки не попадает), и в копии-доноре. Груз-поля (`cargo_*`, вес, срок)
  сохранены.
- **Матч-донорство** (S2-3, вместе с marka): новый вагон, для которого marka не нашла
  груз → ищем в `status6` вагон с `code_station_oper == станция_погрузки`, сверяем вес
  и срок доставки; при совпадении трёх — переносим груз донора вагону-приёмнику, в
  информационное поле пишем номер вагона-донора.
