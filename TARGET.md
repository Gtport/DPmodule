...
**Механизм 1 — `marka`.** ⚠️ ПЕРЕСМОТРЕН владельцем (июль 2026, PR #66/67/81/82) —
реализованная модель отличается от первоначального эскиза:
- идентичность груза (код ЕТСНГ → группа/имя/метка) — отдельный словарь **`cargo`**,
  применяется в Stage 1 каждому вагону независимо от отправителя;
- `marka` — только бизнес-атрибуция: `shipper → gruzotpr, client, sms_1, sms_3, color`;
  ключ `(okpo, station_kod, cargo_GROUP)` — по ГРУППЕ груза, не по коду (новый код
  знакомой группы матчится без правки словаря); `sms_2` не хранится — расчётный
  (`sms_1 + cargo_sms`);
- матч **СТРОГО** по трём полям ключа; частичный матч `(станция+группа)` упразднён
  (подставлял атрибуцию чужого отправителя); несовпавших закрывает наследование по
  единогласному составу (S2-3d, IndexMain) либо донорство status6;
- «снимок вернее потока»: атрибутированные существующие вагоны S2-3 не трогает —
  груз-поля и sms_2 несёт carry-over; правки словарей применяются явной кнопкой
  «Обновить справочники» (`dict_reload`: перезагрузка RAM + гибридный пересчёт).
Нет совпадения → «нет марки» (лог/статистика). Возвращает «заполнена ли атрибуция» —
для перехода к донорству.
...
### 3.19. Запись бизнес-истории `vagon_history` (S2-6)

Запись вех рейса в `vagon_history` (ключ `id` = `dislocation.id` = Vagon+CodeStationNach+
DateNach). **Трейл продвижения (`vagon_operation`) — НЕ здесь**: отдельный внешний
функционал по триггеру `actual↔new`, позже. Пишем **точечно**, не каждую итерацию.

**INSERT** — если `id` отсутствует в `vagon_history` (новый рейс/вагон): полная строка
вех. Если запись впервые появилась уже со статусом 10/12 — сразу поля прибытия/выгрузки.
`invoice_main` фиксируется при вставке и далее НЕ перезаписывается.

**UPDATE** — если `id` есть: точечно, по **переходам** (сравнение `actual` [пред. снимок]
vs `new`):
- `invoice` изменился → пишем `invoice` (текущую накладную; `invoice_main` не трогаем);
- статус сменился → `status`;
- статус `0 → другой` → `index_main` (текущий `record.index_main`);
- переход в `12` → `date_vigr = time_op`, `date_vigr_d` = ЖД-дата (из `date_op_jd`),
  `place_vigr = naznach`;
- переход в `10` → `date_prib = date_kon`, `date_prib_d` = дата(`date_kon`),
  `delay = CalculateDelay(date_prib_d, date_dostav)`, `otkl = CalculateOtkl(date_prib,
  plan_msk)` (пусто без плана), `plan_msk/plan_jd/naznach`.

**Без заморозки после 10** (⚠️ отклонение от gtlogic — там строку статуса 10 замораживали).
`otkl`/`delay`/`plan_*` — формулы заложены, реальные значения появятся с планом подвода
(тяжёлый Stage 4). `date_prib_d` — простая дата; `date_vigr_d` — ЖД-дата (по схеме).

**INSERT/UPDATE-решение:** пакетный `SELECT id FROM vagon_history WHERE id = ANY(...)`
по батчу; полный кэш истории НЕ греем (растёт безгранично). **Место в конвейере:** после
forecast (S2-5), ДО подмены снимка (`actual` = пред. снимок для сравнения).
