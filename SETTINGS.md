...
| `lk_account` | **Аккаунты ЛК РЖД** для автовыгрузки дислокации: ОКПО грузополучателя → логин, подпись потока, вкл/выкл. Пароля здесь НЕТ — его вводит диспетчер при запуске | Сменился логин, добавился порт |
...
**Что делает `correction_coef` (реализовано 14.08.2026).** Он поправляет ТОЛЬКО формулу
интервала очереди причала Stage 4: `min(вагонов, лимит) × 24ч / (pc × correction_coef)`.
Смысл — развести **норматив** и **факт**: `pc` остаётся документной перерабатывающей
способностью (её же показывает «Грузовая работа»), а коэффициент говорит, во сколько раз
станция реально принимает больше или меньше. `> 1` — принимают больше норматива, очередь
сжимается, прогноз прибытия становится раньше; `< 1` — наоборот. Отсутствие строки профиля
и дефолтная единица дают прежнее поведение бит-в-бит, поэтому боевые станции не задеты.
Коэффициент применяется и на вкладке what-if «Прогноз прибытия/выгрузки» — там те же числа.
⚠️ Он НЕ моделирует простой в пути (узловые сортировки): это событие, а не скорость.

Живой пример — станция АМУР клиента АНБ: `pc = 24` по документам порта, фактически подают
в 2–2,5 раза больше, поэтому `correction_coef = 2.0` (`scripts/clients/river.sql`).
